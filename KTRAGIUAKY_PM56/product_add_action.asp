...
<%
	...
	sql="select * From product_0201366_6 where pname='" & pname &"'"
	rs.open sql, conn 
	if (not rs.eof) then
		Session("product_add_error")="Tên sản phẩm: " & pname &" đã có rồi!"
		Response.Redirect("product_add.asp")	
    else 
		sql = "insert into product_0201366_6(pname,pdesc,pimage,cid,pstatus,pprice,pquantity) values ('" & pname & "','" & pdesc & "','" & pimage & "'," & cid & "," & pstatus & "," &pprice & "," & pquantity & ")"
		'Response.write(sql)
		conn.execute sql 
		conn.close
